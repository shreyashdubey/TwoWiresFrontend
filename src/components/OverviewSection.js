...
const OverviewSection = ({ published, submitted }) => {
  ...
  useEffect(() => {
    ...
    if (!store) {
      ...
      try {
        // Make the API request to fetch contest description
        instance
          .get(`/api/contest/get-contest-by-id?contestId=${contestId}`)
          .then((response) => {
            // Check if the API request was successful
  ...
  const handleSaveOverview = async () => {
    ...
    try {
      ...
      const response = await instance.post(
        "/api/contest-description/create-contest-description",
        {
          admin,
          contestId,
          subtitle,
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await instance.put(
        `/api/contest-description/edit-contest-description/${contestId}`,
        { subtitle, evaluation, timeline, overview, description, tags },
        {
          "Content-Type": "application/json",
        },
      );
      ...
      if (response.success) {
        console.log(
          "Contest description saved successfully:",
          response.contestDescription,
        );
        setOverviewSaved(true);
        setInitialFetch(false);
        setIsEditing(false);
  ...
  const handleContestUpdate = async (e) => {
    ...
    try {
      ...
      const response = await instance.put(
        `/api/contest/edit-contest/${contestId}`,
        { contestCreator: userId, isSubmitted: true },
        { "Content-Type": "application/json" },
      );
      ...
      <Box p={4} w="100%" mb="100px">
        {(isSaved || store) && !submitButton && !published && !submitted ? (
          <Button
            ...
            colorScheme="teal"
            size="md"
            onClick={handleContestUpdate}
            ml={["95px", "90px", "100px", "110px", "1302px", "190px"]}
          >
