...
import { GET_USERID, INVITE_USER, GET_TEAM_MEMBER } from "../utils/endpoints";
...
const InviteUsers = () => {
  ...
  const handleInviteUser = async (e) => {
    ...
    try {
      ...
      const response_team = await instance.post(
        INVITE_USER,
        { reciever, sender, team },
      ...
      <Center>
        <Box p={4} mt="150px" w="50%" borderColor="red.900">
          <Heading
            ...
            mb={4}
            color="custom.white"
            borderColor="custom.darkSlateBlue"
          >
          ...
          <Table variant="simple" mt={4}>
            <Thead>
              <Tr>
                <Th>
                  <Text color="custom.white">User</Text>
                </Th>
                <Th>
                  <Text color="custom.white">Status</Text>
                </Th>
                <Th>
                  <Text color="custom.white">Action</Text>
                </Th>
              </Tr>
            </Thead>
            <Tbody>
              {invitedUsers.map((user, index) => (
                <Tr key={index}>
                  <Td>
                    <Text color="custom.white">{user.username}</Text>
                  </Td>
                  <Td>
                    <Text color="custom.white">{user.inviteStatus}</Text>
                  </Td>
                  <Td>
                    <Button
                      colorScheme="red"
