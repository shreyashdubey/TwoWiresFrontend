...
import { ADD_CONTEST_PLAN } from '../utils/endpoints';
...
import { GET_ALL_PLAN } from '../utils/endpoints';
import { ADD_EXECUTION_STEP } from '../utils/endpoints';
  ...
  const handleSaveExecutionStep = async() => {
    ...
    try {
      ...
      const response = await instance.post(
        `${ADD_EXECUTION_STEP}`,
        {
            planId , stepNumber , stepName   , stepDescription ,
        },
    ...
    // Associate execution step with the current plan using currentPlanIndex
    ...
    <Box p={4}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Box mb={4}>
              ...
              <ReactQuill value={planName} onChange={(value) => setPlanName(value)} />
            </Box>
            <Box>
              <Text mb={2}>Plan Content:</Text>
              <ReactQuill value={planContent} onChange={(value) => setPlanContent(value)} />
            </Box>
          </ModalBody>
          <ModalFooter>
      ...
      {plans.map((plan, index) => (
        <Card key={index} my={4} p={4}>
              ...
              <Spacer/>
              <Flex       w="10%">
                  <Button variant="solid" size="sm" onClick={() => handleUpvote(index)}>
                    <Image src= {upvote} alt="Upvote" boxSize="20px" />
                  </Button>
                  <Button variant="solid" size="sm" onClick={() => handleDownvote(index)}>
          ...
          <HStack>
          <Text fontSize="md" fontWeight="bold" >Plan Content:</Text>
